...
You are operating as an SDD (Specification-Driven Development) planner.
Your task is to generate the **plan.md** file for the RAG Chatbot System, strictly aligned
with the Constitution and the Specification already created.

The `plan.md` must break down the entire RAG Chatbot project into a sequenced, milestone-based,
implementation-ready plan. It must not include implementation code. It must not contradict any
technical or functional constraints in the Constitution or Specification.
...
- Why sequenced execution is required
- Dependencies between major components
- Any prerequisites for project setup (FastAPI, uv, Qdrant, Neon, ChatKit)

# 2. HIGH-LEVEL ARCHITECTURE RECAP
Summaries (not specs) of:
...
Break the work into clear milestones with deliverables.

Example structure (you must expand with more detail):
...
Deliverables:
- Text preprocessing rules implemented
- Chunker created (800–1200 token chunks)
- Cohere embedding service built
- Write embeddings + metadata to Qdrant + Neon
- Background task support defined

## Milestone 4 — Retrieval Pipeline
Deliverables:
- Vector similarity search
- Metadata retrieval from Neon
- Combined ranked retrieval output format
- Edge cases defined (no results, empty context, truncated answers)
...
- Agent with tool definitions:
  ...
  - Neon data fetch tool
- Context assembly pipeline
- Hallucination guardrails
- Answer formatting + citation rules
...
- API binding to FastAPI backend
- Source passage viewer

## Milestone 8 — Evaluation, Optimization, Testing
Deliverables:
- Latency profiling (target: <1.5 sec)
- Retrieval quality tests
- Edge case tests
- Cohere embedding performance validation
- Failure mode handling
- Free-tier resource checks

## Milestone 9 — Deployment
Deliverables:
- FastAPI backend deployed using a low-cost service:
  - Railway / Render / Fly.io (must support Python + uv)
- Environment variables configured for:
  - Cohere API key
  - Qdrant API key + URL
  - Neon connection string
- UI deployed (ChatKit frontend)
- Domain + HTTPS ensured (if supported)

# 4. DETAILED TASK BREAKDOWN PER MILESTONE
List granular tasks for each milestone.
Each task must:
- Be actionable
- Produce a defined outcome
- Depend on the previous milestone when needed

# 5. TIMELINE (ABSTRACT)
Provide:
- Approximate timeline (not tied to real-world dates)
- Parallelizable vs sequential work
- Critical-path tasks

# 6. RISK ANALYSIS & MITIGATION PLAN
Include:
- Free tier limitations (Qdrant/Neon/Cohere)
- Latency risks
- Deployment failure risks
- Cloud credential rotation risks
- Optimization strategies (caching, fewer tokens, tight chunking)

# 7. ACCEPTANCE CRITERIA
Define completion requirements:
- Chatbot answers grounded strictly in book text
- “Selected text only” mode works reliably
- Retrieval accuracy validated
- UI and backend integrated
- Agents SDK routing functioning
- All components pass functional + integration tests

FORMAT:
Produce plan.md in clean, multi-level Markdown with headings, lists, sublists, and tables.
Do not include code.
Stay fully aligned with Constitution and Specification.

Output in Chatbot_specs Folder:
- File Type: Markdown
- File Path: Chatbot_specs/plan.md.
