"""
Safe script to crawl Docusaurus URLs and generate embeddings for the Physical AI Humanoid Robotics Book.
This version includes additional resource management to prevent system overload.
"""
import asyncio
import os
import uuid
...
async def crawl_and_generate_embeddings():
    ...
    connector = aiohttp.TCPConnector(limit=5, limit_per_host=2)  # Reduced from 10 to 5
            ...
            if group_name in batch_groups:
                ...
                for i, url_info in enumerate(group_urls):
                    ...
                    if memory_percent > 80:
                        ...
                        await asyncio.sleep(2)  # Increased delay to be more conservative
                        gc.collect()
                        # Check memory again after cleanup
                        memory_percent = psutil.virtual_memory().percent
                        if memory_percent > 90:
                            logger.error(f"    ! Critical memory usage: {memory_percent:.1f}%. Pausing to prevent system crash...")
                            await asyncio.sleep(10)  # Longer pause for critical memory usage
                            gc.collect()
                    ...
                    try:
                        ...
                        if current_memory > 90 or current_cpu > 95:
                            ...
                            await asyncio.sleep(15)  # Longer pause for critical resources
                            gc.collect()
                        ...
                        if content_size > 5 * 1024 * 1024:  # Reduced from 10MB to 5MB
                        ...
                        try:
                            ...
                            result = await asyncio.wait_for(
                                ...
                                timeout=900  # Increased timeout to 15 minutes per content item
                            )
                        except asyncio.TimeoutError:
                            logger.error(f"    X Timeout: Processing took more than 15 minutes")
                            failed_pages += 1
                            continue
                        except Exception as e:
